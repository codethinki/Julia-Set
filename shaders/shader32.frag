...
void main(){
	float realZ = map(gl_FragCoord.x, 0, push.general.x, push.edges[0], push.edges[1]), imgZ = map(gl_FragCoord.y, 0, push.general.y, push.edges[2], push.edges[3]), realC = push.cOffset.x, imgC = push.cOffset.y;
	int i = -1;
